...
<template>
  <div class="relative">
    ...
    <transition name="slide-fade" mode="out-in">
      <p class="text-xs" v-if="errorMessage">{{ errorMessage }}</p>
    </transition>
  </div>
</template>
...
export default {
  ...
  setup({ min, edit, startValue, inserted }, { emit }) {
    const schema = computed(() =>
      yup.object({
        campo: yup
          .string()
          .required(startValue)
          .min(min, `Insira uma senha valida`),
      })
    );
    useForm({
    ...
    watch([value, errorMessage], () => {
      emit("invalid-input", { field: "password", value: errorMessage.value });
    });
